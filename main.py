...
def parse_duration(s: str) -> int:
    ...
    days = max(0, int(m.group("d") or 0))
    hours = max(0, int(m.group("h") or 0))
    minutes = max(0, int(m.group("m") or 0))
    seconds = max(0, int(m.group("s") or 0))
...
class GiveawayBot(commands.Bot):
    ...
    async def on_message(self, message: discord.Message):
        ...
        for g in list(self.giveaways.values()):
            if g.guild_id != message.guild.id or g.ended:
                continue
            if g.must_said and match_phrase(content, g.must_said):
    ...
    async def _end_giveaway(self, message_id: int, force: bool = False) -> Optional[List[int]]:
        ...
        if guild is not None:
            for uid in g.reacted_users:
                ...
                if member and not member.bot and (g.must_said is None or uid in g.said_users):
        ...
        if not pool:
            cond_text = f"需按 {EMOJI_JOIN}" if g.must_said is None else \
                        f"需發言「{g.must_said}」並按 {EMOJI_JOIN}"

            if channel:
                await channel.send(f"🎁 **{g.prize}** 抽獎結束！沒有符合資格的參加者（{cond_text}）。")
            ...
            cond_text = f"只需按 {EMOJI_JOIN}" if g.must_said is None else \
                        f"說過「{g.must_said}」且按 {EMOJI_JOIN}"
        ...
        try:
            if channel:
                ...
                if msg.embeds:
                    ...
                    for f in e.fields:
                        ended_embed.add_field(name=f.name, value=f.value, inline=f.inline)
    ...
    async def _countdown_and_end(self, message_id: int):
        ...
        delay = max(0, g.ends_at_unix - now_taiwan().timestamp())
        try:
...
@app_commands.describe(
    duration="持續時間（例如：30m / 2h / 1h30m / 1d2h）",
    prize="獎品名稱",
    winners="得獎人數（預設 1）",
    must_said="（可選）需說出的訊息，留空則無條件",
    channel="發佈抽獎的頻道（預設當前頻道）",
)
async def gstart(
    ...
    must_said = must_said.strip() if must_said else None
    ...
    if not isinstance(ch, discord.TextChannel):
        return await interaction.response.send_message("指定的不是文字頻道。", ephemeral=True)

    ends_at = now_taiwan().timestamp() + seconds

    cond_text = f"在任一頻道說過「`{must_said}`」" if must_said else "無條件，只需按 🎉"

    e = discord.Embed(
        title="🎉 抽獎開始！",
        description=(
            f"獎品：**{prize}**\n"
            f"主辦：{interaction.user.mention}\n"
            f"得獎人數：**{winners}**\n"
    ...
    asyncio.create_task(bot._countdown_and_end(msg.id))

    await interaction.followup.send(
        f"已建立抽獎（訊息 ID：`{msg.id}`）。",
        ephemeral=True
    )
...
async def gend(interaction: discord.Interaction, message_id: str):
    ...
    try:
        ...
        return await interaction.response.send_message("message_id 應為數字。", ephemeral=True)

    res = await bot._end_giveaway(mid, force=True)
    if res is None:
        return await interaction.response.send_message("找不到該抽獎或已結束。", ephemeral=True)
...
async def greroll(
    ...
    if not interaction.user.guild_permissions.manage_guild:
        return await interaction.response.send_message(
            "需要 **管理伺服器** 權限才能重抽。", ephemeral=True
        )
    ...
    try:
        ...
        return await interaction.response.send_message(
            "message_id 應為數字。", ephemeral=True
        )

    g = bot.giveaways.get(mid)
    if not g:
        return await interaction.response.send_message("找不到該抽獎", ephemeral=True)

    guild = bot.get_guild(g.guild_id)
    if guild is None:
        return await interaction.response.send_message("找不到伺服器資訊。", ephemeral=True)
    ...
    if not eligible_ids:
        return await interaction.response.send_message("目前沒有符合資格的參加者。", ephemeral=True)
